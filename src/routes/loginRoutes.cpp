    ...
    CROW_ROUTE(app, "/driver")
        .methods(
            ...
            try {
                ...
                pqxx::result r =
                    txn.exec("SELECT user_id FROM sessions WHERE token = " +
                             txn.quote(session_id) + " AND expires_at > NOW()");
                ...
                r = txn.exec(
                    "SELECT d.id, d.name, t.name AS team_name, d.points FROM "
                    "drivers d JOIN teams t ON d.team_id = t.id WHERE d.id = " +
                    txn.quote(driver_id));
                if (r.size() == 1) {
            ...
            } catch (const std::exception& e) { x["error"] = e.what(); }
            res.write(x.dump());
            res.end();
        });
