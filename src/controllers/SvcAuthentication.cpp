...
bool SvcAuthentication::verifyLoginDetails() const {
    try {
        ...
        std::string query = "SELECT COUNT(*) FROM users WHERE username = " + txn.quote(m_username) + " AND password = " + txn.quote(m_password);
...
std::string SvcAuthentication::generateSessionToken() const {
    ...
    try {
        ...
        // Get the user ID
        std::string query = "SELECT id FROM users WHERE username = " + txn.quote(m_username);
        pqxx::result r = txn.exec(query);
        if (r.size() != 1) {
        ...
        auto expires_at = std::chrono::system_clock::to_time_t(now + std::chrono::hours(1));

        // Insert the session token into the sessions table
        query = "INSERT INTO sessions (user_id, token, expires_at) VALUES (" + txn.quote(user_id) + ", " + txn.quote(token) + ", to_timestamp(" + txn.quote(std::to_string(expires_at)) + "))";
        txn.exec(query);
        txn.commit();
    } catch (const std::exception &e) {
        std::cerr << "Database error: " << e.what() << std::endl;
